...
describe('configuration', function() {
    ...
    describe('.json', function() {
        ...
        it('neatly indents the JSON', function() {
            const config = configuration();
            config.resource.aws_s3_bucket.abc = { bucket: 'test' };
            expect(config.json).toMatch(/^{\n\t"resource": {\n\t\t"aws_s3_bucket": {/);
        });
    
        it('does not include empty sections', function() {
            const config = configuration();
            const json = config.json;
            expect(json).toBe('{}');
        });

        it('does not include null properties', function() {
            const config = configuration();
            config.resource.aws_s3_bucket.empty = { bucket: 'test', something: null };
            expect(config.json).not.toContain('something');
            expect(config.json).not.toContain('null');
        });
    });
    ...
    describe('.writeTo', function() {
        beforeEach(function() {
            this.tempFile = path.join(os.tmpdir(), 'testConfigWriteTo.tf.json');
        });

        afterEach(function() {
            return deleteFile(this.tempFile).catch(err => {
                if (err.code !== 'ENOENT') console.error(err);
            });
        });

        it('returns promise', function(completeTest) {
            const config = configuration();
            // Explicitly use .then to see if writeTo returned a thenable
            config.writeTo(this.tempFile).then(completeTest);
        });

        it('writes JSON to specified file name', async function() {
            const config = configuration();
            config.module.something = { source: 'x' };

            await config.writeTo(this.tempFile);

            const fromFile = JSON.parse(await readFile(this.tempFile));
            expect(fromFile).toEqual(jasmine.objectContaining({
                module: {
                    something: {
                        source: 'x'
                    }
